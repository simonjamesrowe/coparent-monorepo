...
import type { Expense, Category, Account, Statement, StatementLine } from '../../types/expenses';
...
export function ExpenseList({
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter((expense) => {
      ...
      if (selectedAccountId !== 'all') {
        const line = expense.sourceStatementLineId
          ? statementLineLookup[expense.sourceStatementLineId]
          : undefined;
      ...
      if (search) {
        const value =
          `${expense.description} ${childLookup[expense.childId ?? ''] ?? ''}`.toLowerCase();
  ...
  return (
    <section className="min-h-screen bg-slate-50 px-6 py-10 text-slate-900 sm:px-8 dark:bg-slate-950 dark:text-slate-100">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-300">
                Filter by account, category, or date range. Drill into specific child expenses and
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      {expense.date} • {parentLookup[expense.createdByParentId]}
                    </p>
                  </div>
                  <span
                    className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-semibold ${colorClassMap[category?.color ?? 'stone']}`}
                  >
                  ...
                  <span className="text-sm text-slate-600 dark:text-slate-300">
                    {expense.childId ? childLookup[expense.childId] : 'Shared'}
                  </span>
                  <span
                    className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-semibold ${statusClassMap[expense.status]}`}
                  >
