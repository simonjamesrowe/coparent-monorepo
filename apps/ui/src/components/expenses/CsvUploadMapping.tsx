...
import type { Account, CsvMappingTemplate, Statement, StatementLine } from '../../types/expenses';
...
export interface CsvUploadMappingProps {
  accounts: Account[];
...
export function CsvUploadMapping({
  ...
  const accountStatements = useMemo(
    () => statements.filter((s) => s.accountId === selectedAccountId),
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-300">
                Upload a statement, align the columns with our expense fields, and save the mapping
                template for this account.
              </p>
            </div>
            <button
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_1.9fr]">
          <aside className="space-y-6">
            <div className="rounded-3xl border border-slate-200/70 bg-white p-6 shadow-sm dark:border-slate-800/70 dark:bg-slate-900">
              <h2 className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-400 dark:text-slate-500">
                Accounts
              </h2>
              ...
              <div className="mt-4 space-y-2">
                {templates.length ? (
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    No templates yet. Upload a statement to create one.
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-300">
                    Assign each required field to a column from your CSV.
                  </p>
                </div>
                {latestStatement && (
            ...
            <div className="rounded-3xl border border-slate-200/70 bg-white p-6 shadow-sm dark:border-slate-800/70 dark:bg-slate-900">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  Showing {previewLines.length} rows
                </span>
              </div>
              <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
                ...
                {previewLines.length ? (
                  ...
                  <div className="px-4 py-6 text-sm text-slate-500 dark:text-slate-400">
                    No statement lines available yet.
                  </div>
                )}
              </div>
            </div>
