...
import type { Budget, Category } from '../../types/expenses';

const formatCurrency = (value: number) => `$${value.toFixed(2)}`;

export interface BudgetSetupProps {
  budgets: Budget[];
  categories: Category[];
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-300">
                Establish category limits and monitor current spend to stay aligned on family
                spending goals.
              </p>
            </div>
            <select
        ...
        <div className="rounded-3xl border border-slate-200/70 bg-white p-6 shadow-sm dark:border-slate-800/70 dark:bg-slate-900">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {monthBudgets.map((budget) => (
              <div
                key={budget.id}
                className="grid grid-cols-[1.4fr_1fr_1fr_1fr] items-center gap-3 py-4 text-sm"
              >
                <div>
                  <p className="font-semibold text-slate-900 dark:text-slate-100">
                    {categoryLookup[budget.categoryId]}
                  </p>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDraftLimits((prev) => ({
                      ...prev,
                      [budget.categoryId]: Number(e.target.value),
                    }))
                  }
