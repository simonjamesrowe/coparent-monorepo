...
import type { DashboardSummary, Category, Budget, Account } from '../../types/expenses';
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-300">
                A quick pulse on where money goes, how budgets are tracking, and what reimbursements
                are outstanding.
              </p>
            </div>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {[
              { label: 'Total spent', value: formatCurrency(dashboardSummary.totalSpent) },
              {
                ...
                value: formatCurrency(dashboardSummary.totalChildExpenses),
              },
              {
                label: 'Reimbursements due',
                value: formatCurrency(dashboardSummary.reimbursementsDue),
              },
            ].map((card) => (
              <div
                key={card.label}
          ...
          <div className="rounded-3xl border border-slate-200/70 bg-white p-6 shadow-sm dark:border-slate-800/70 dark:bg-slate-900">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">
                {dashboardSummary.categoryBreakdown.length} categories
              </span>
            </div>
            <div className="mt-6 space-y-5">
              {dashboardSummary.categoryBreakdown.map((breakdown) => {
                ...
                const limit =
                  breakdown.budgetLimit ??
                  budgetLookup[`${breakdown.categoryId}-${dashboardSummary.month}`] ??
                  null;
                ...
                return (
                  <div key={breakdown.categoryId} className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-xs text-slate-500 dark:text-slate-400">
                        {formatCurrency(breakdown.spent)}{' '}
                  ...
                  >
                    <div>
                      <p className="font-semibold text-slate-800 dark:text-slate-100">
                        {accountLookup[account.accountId] ?? 'Account'}
                      </p>
              ...
              <div className="mt-4 space-y-3">
                {budgets.map((budget) => (
                  <div key={budget.id} className="flex items-center justify-between text-sm">
                    <div>
                      <p className="font-semibold text-slate-800 dark:text-slate-100">
                        {categoryLookup[budget.categoryId]?.name ?? 'Category'}
                      </p>
