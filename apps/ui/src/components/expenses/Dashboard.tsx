...
function formatMonth(monthStr: string): string {
  const [year = '2025', month = '1'] = monthStr.split('-');
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200/60 bg-white/80 shadow-lg shadow-slate-200/30 backdrop-blur-sm dark:border-slate-700/60 dark:bg-slate-800/60 dark:shadow-slate-900/30">
              <div className="flex items-center justify-between border-b border-slate-200/60 p-5 dark:border-slate-700/60">
              ...
              <div className="space-y-4 p-5">
                {dashboardSummary.categoryBreakdown.map((item) => {
                  ...
                  const percentage = item.budgetLimit
                    ? Math.min((item.spent / item.budgetLimit) * 100, 100)
                    : 0;
                    ...
                    <div key={item.categoryId} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div
                            className={`h-8 w-8 rounded-lg ring-1 ${styles.ring} ${styles.bg} flex items-center justify-center`}
                          >
                        ...
                        <div className="text-right">
                          <p
                            className={`text-sm font-semibold ${isOverBudget ? 'text-rose-600 dark:text-rose-400' : 'text-slate-700 dark:text-slate-200'}`}
                          >
                ...
                {dashboardSummary.accountSpend.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`flex h-10 w-10 items-center justify-center rounded-xl ${account.type === 'credit-card' ? 'bg-rose-500/15' : 'bg-teal-500/15'}`}
                        >
          ...
          <div className="space-y-6">
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex min-w-0 items-start gap-3">
                          <div
                            className={`h-8 w-8 rounded-lg ring-1 ${styles.ring} ${styles.bg} mt-0.5 flex flex-shrink-0 items-center justify-center`}
                          >
                            <span className={`text-xs font-bold ${styles.text}`}>
                              {category?.name.charAt(0) || '?'}
                            </span>
            ...
            {pendingExpenses.length > 0 && (
              <div className="rounded-2xl border border-amber-200/60 bg-amber-50/80 shadow-lg shadow-amber-200/20 backdrop-blur-sm dark:border-amber-700/40 dark:bg-amber-900/20 dark:shadow-amber-900/20">
                <div className="border-b border-amber-200/60 p-5 dark:border-amber-700/40">
                  ...
                  {pendingExpenses.slice(0, 3).map((expense) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => onViewExpense?.(expense.id)}
                        className="w-full rounded-xl border border-amber-200/50 bg-white/60 p-3 text-left transition-colors hover:bg-white/80 dark:border-amber-700/30 dark:bg-slate-800/40 dark:hover:bg-slate-800/60"
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="truncate text-sm font-medium text-slate-700 dark:text-slate-200">
