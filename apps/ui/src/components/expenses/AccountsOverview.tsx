        ...
        <header className="rounded-3xl border border-slate-200/70 bg-gradient-to-br from-white via-slate-50 to-slate-100 p-8 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.45)] dark:border-slate-800/70 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.4em] text-teal-600 dark:text-teal-400">
                Accounts
              </p>
              <h1 className="text-3xl font-semibold sm:text-4xl">Family finance accounts</h1>
              <p className="max-w-2xl text-sm text-slate-600 dark:text-slate-300">
                Track every bank account and card connected to the family. Statements, templates,
                and uploads stay tied to each account.
              </p>
            </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-200">
                      {account.type === 'credit-card'
                        ? cardTypeLabel[account.cardType ?? 'visa']
                        : (account.accountSubtype ?? 'standard')}
                    </span>
                    <span className="text-xs">Owner: {owner?.name ?? 'Unassigned'}</span>
                    <span className="text-xs">
                      Statements: {statementCountByAccount[account.id] ?? 0}
                    </span>
                    <span className="text-xs">
                      Templates: {templatesCountByAccount[account.id] ?? 0}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    <button
                    ...
                    <button className="rounded-full border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-600 transition hover:border-rose-200 hover:text-rose-500 dark:border-slate-700 dark:text-slate-300">
                      View activity
                    </button>
                  </div>
                </div>
