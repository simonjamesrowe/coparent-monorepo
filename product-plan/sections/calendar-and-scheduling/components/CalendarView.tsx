...
export function CalendarView({
  ...
  const handleNavigate = (direction: 'prev' | 'next' | 'today') => {
    ...
    if (direction === 'today') {
      newDate = new Date(2025, 0, 1) // Using sample data date
    } else {
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Activities', value: events.filter(e => e.type === 'activity').length, suffix: 'scheduled' },
            { label: 'Medical', value: events.filter(e => e.type === 'medical').length, suffix: 'appointments' },
